...
async def upgrade(db: BaseDBAsyncClient) -> str:
    """Add recipe visibility, enabled state, and optional owner columns."""
    _, rows = await db.execute_query('PRAGMA table_info("recipe")')
    ...
    if "owner_id" not in columns:
        statements.append(
            'ALTER TABLE "recipe" ADD COLUMN "owner_id" INT REFERENCES "user" ("id") ON DELETE CASCADE'
        )
