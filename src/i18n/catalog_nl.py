"""Dutch UI text catalog keyed for database seeding."""

TEXTS: dict[str, str] = {
    # app
    "app.name": "Weekmenu",
    "app.title.default": "Weekmenu",
    "app.title.login": "Log in · Weekmenu",
    "app.title.register": "Registreren  · Weekmenu",
    "app.title.force_password": "Wachtwoord instellen · Weekmenu",
    "app.title.account": "Profiel",
    "app.title.find_recipe": "Recept zoeken",
    "app.title.add_recipe": "Recept toevoegen",
    "app.title.edit_recipe": "Wijzig {recipe_name}",
    "app.title.view_recipe": "{recipe_name}",
    "app.title.recipes_missing_tags": "Recepten zonder tags",
    "app.title.recipes_missing_description": "Recepten zonder beschrijving",
    "app.title.random_public_recipe": "Willekeurig publiek recept",
    "app.title.week_menu": "Weekmenu",
    "app.title.grocery_list": "Boodschappen",
    "app.title.manage_tag_groups": "Tags beheren",
    "app.title.manage_shops": "Winkels beheren",
    "app.title.manage_week_menu_constraints": "Beheer tagbeperkingen",
    "app.title.weekly_groceries": "Wekelijkse boodschappen",
    "app.title.inventory": "Voorraad",
    "app.title.units": "Eenheden",
    "app.title.merge_ingredient_units": "Ingrediënt-eenheden samenvoegen",
    "app.title.merge_ingredients": "Ingrediënten samenvoegen",
    "app.title.admin_users": "Gebruikers",
    "app.title.admin_translations": "Vertalingen",
    "app.title.admin_info": "Technische info",
    # nav
    "nav.week_menu": "Weekmenu",
    "nav.grocery_list": "Boodschappen",
    "nav.inventory": "Voorraad",
    "nav.recipes": "Recepten",
    "nav.find_recipe": "Recept zoeken",
    "nav.add_recipe": "Recept toevoegen",
    "nav.random_private_recipe": "Willekeurig eigen recept",
    "nav.random_public_recipe": "Willekeurig publiek recept",
    "nav.missing_tags": "Zonder tags",
    "nav.missing_description": "Zonder beschrijving",
    "nav.settings": "Opties",
    "nav.tag_groups": "Tags beheren",
    "nav.shops": "Winkels",
    "nav.week_menu_constraints": "Weekmenu beperkingen",
    "nav.weekly_groceries": "Wekelijkse boodschappen",
    "nav.units": "Eenheden",
    "nav.merge_ingredient_units": "Ingrediënt-eenheden samenvoegen",
    "nav.merge_ingredients": "Ingrediënten samenvoegen",
    "nav.account": "Profiel",
    "nav.logout": "Uitloggen",
    "nav.admin": "Beheer",
    "nav.users": "Gebruikers",
    "nav.translations": "Vertalingen",
    "nav.info": "Technische info",
    # home
    "home.lead": "Welkom! Plan je maaltijden, stel je kookboek samen, en ontdek wat je morgen eet.",
    "home.week_menu.desc": "Stel een willekeurig weekmenu samen.",
    "home.grocery_list.desc": "Bekijk je boodschappen per winkel en vink af wat je al hebt.",
    "home.find_recipe.desc": "Zoek op naam, bereiding of ingrediënt.",
    "home.add_recipe.desc": "Voeg een nieuw gerecht toe met ingrediënten en bereiding.",
    "home.random_private_recipe.desc": "Zoek een willekeurig gerecht uit je eigen kookboek.",
    "home.random_public_recipe.desc": "Zoek een willekeurig publiek gerecht.",
    "home.missing_tags.desc": "Zoek gerechten die tags in één of meer groepen missen.",
    "home.missing_description.desc": "Zoek gerechten die nog een beschrijving missen.",
    "home.tag_groups.desc": "Beheer tag-groepen zoals seizoen en koolhydraattype.",
    "home.shops.desc": "Koppel ingrediënten aan winkels om lijsten te groeperen.",
    "home.weekly_groceries.desc": "Beheer ingrediënten die je elke week nodig hebt los van je weekmenu.",
    "home.inventory.desc": "Houd bij wat je in huis hebt. Nieuwe boodschappenlijsten slaan over wat al op voorraad is.",
    "home.units.desc": "Beheer eenheden (afkortingen, enkelvoud, meervoud) voor ingrediënten.",
    "home.merge_ingredient_units.desc": "Zet recepten om die verschillende eenheden voor hetzelfde ingrediënt gebruiken.",
    "home.merge_ingredients.desc": "Voeg dubbele ingrediënten zoals olie en olijfolie samen onder één naam.",
    "home.account.desc": "Beheer je account: email, wachtwoord, en meer.",
    "home.users.desc": "Beheer gebruikersaccounts en adminrechten.",
    "home.translations.desc": "Bewerk UI-teksten per taal.",
    "home.info.desc": "Bekijk database- en runtimegegevens.",
    # auth
    "auth.login.subtitle": "Log in om je week te plannen.",
    "auth.register.subtitle": "Maak een account aan.",
    "auth.force_password.subtitle": "Kies een nieuw wachtwoord om verder te gaan.",
    "auth.force_password.action": "Wachtwoord opslaan",
    "auth.field.username": "Gebruiker",
    "auth.field.password": "Wachtwoord",
    "auth.field.email": "E-mail",
    "auth.field.optional": "(optioneel)",
    "auth.field.confirm_password": "Bevestig wachtwoord",
    "auth.action.login": "Inloggen",
    "auth.action.create_account": "Registreren",
    "auth.switch.no_account": "Nog geen account?",
    "auth.switch.create_one": "Maak een account aan.",
    "auth.switch.have_account": "Heb je al een account?",
    "auth.switch.log_in": "Log in",
    # profile
    "profile.lead": "Beheer je profiel.",
    "profile.section.details": "Details",
    "profile.username_label": "Gebruikersnaam:",
    "profile.action.save_email": "E-mailadres opslaan",
    "profile.section.settings": "Instellingen",
    "profile.field.language": "Taal",
    "profile.field.default_servings": "Standaard aantal eters",
    "profile.action.save_settings": "Instellingen opslaan",
    "profile.section.change_password": "Wijzig wachtwoord",
    "profile.field.current_password": "Huidig wachtwoord",
    "profile.field.new_password": "Nieuw wachtwoord",
    "profile.field.confirm_new_password": "Bevestig nieuw wachtwoord",
    "profile.action.change_password": "Wachtwoord wijzigen",
    "profile.section.danger_zone": "Gevarenzone",
    "profile.confirm.delete_account": "Weet je zeker dat je je profiel en alle recepten wilt verwijderen? Je kunt dit niet ongedaan maken.",
    "profile.action.delete_account": "Account verwijderen",
    # common
    "common.search": "Zoeken",
    "common.servings": "Eters",
    "common.description": "Bereiding",
    "common.ingredients": "Ingrediënten",
    "common.tags": "Tags",
    "common.save": "Opslaan",
    "common.edit": "Wijzigen",
    "common.delete": "Verwijderen",
    "common.cancel": "Annuleren",
    "common.add": "Toevoegen",
    "common.remove": "Verwijderen",
    "common.confirm": "Bevestigen",
    "common.dismiss": "Verbergen",
    "common.any": "Maakt niet uit",
    "common.unit": "Eenheid",
    "common.name": "Naam",
    "common.amount": "Hoeveelheid",
    "common.quantity": "Kwantiteit",
    "common.background": "Achtergrond",
    "common.text": "Tekst",
    "common.copy": "Kopiëren",
    "common.include_public_recipes": "Laat ook publieke recepten zien",
    "common.placeholder.search_recipe": "Naam, bereiding of ingredient.",
    "common.status.private": "Privé",
    "common.status.public": "Publiek",
    "common.status.enabled": "Aan",
    "common.status.disabled": "Uit",
    "common.aria.recipe_metadata": "receptstatus",
    "common.aria.recipe_tags": "recepttags",
    "common.empty.no_tag_groups": "Geen tag-groepen gevonden. Voeg ze toe op de tag-beheerpagina.",
    "common.empty.no_tag_groups_short": "Geen tag-groepen gevonden.",
    "common.yes": "Ja",
    "common.no": "Nee",
    # day
    "day.monday": "Maandag",
    "day.tuesday": "Dinsdag",
    "day.wednesday": "Woensdag",
    "day.thursday": "Donderdag",
    "day.friday": "Vrijdag",
    "day.saturday": "Zaterdag",
    "day.sunday": "Zondag",
    # search
    "search.lead": "Zoek in je eigen kookboek op naam, bereiding, of ingrediënt.",
    "search.filter_by_tags": "Filter op tags",
    "search.filter_hint": "Recepten zonder een tag in een groep worden altijd meegenomen in die groep.",
    "search.placeholder.waiting": "Resultaten worden getoond als je hier typt.",
    "search.results.title": "Zoekresultaten",
    "search.results.empty": "Er zijn geen recepten gevonden op basis van je zoekopdracht.",
    # recipe
    "recipe.add.lead": "Vul de gegevens hieronder in. Je kunt ingrediënten één voor één toevoegen.",
    "recipe.field.name": "Naam recept",
    "recipe.field.prep_time_minutes": "Voorbereidingstijd (minuten)",
    "recipe.field.cook_time_minutes": "Kooktijd (minuten)",
    "recipe.field.image_url": "Afbeeldings-URL",
    "recipe.field.image_url_hint": "Optionele link naar een .jpg-, .jpeg-, .png-, .gif-, .webp- of .avif-afbeelding (https bij voorkeur).",
    "recipe.field.image_url_optional": "(optioneel)",
    "recipe.add.ingredients_hint": "Voeg hoeveelheid, eenheid en naam toe voor elk ingrediënt.",
    "recipe.add.action.add_ingredient": "(+) Ingrediënt toevoegen",
    "recipe.add.tags_hint": "Selecteer tags die dit recept omschrijven.",
    "recipe.add.submit": "Recept toevoegen",
    "recipe.add.success": "Recept {recipe_name} toegevoegd aan je kookboek.",
    "recipe.add.view_recipe": "Recept bekijken",
    "recipe.edit.back_to_view": "Recept bekijken",
    "recipe.edit.mode_badge": "Aan het bewerken",
    "recipe.edit.section.title": "Titel",
    "recipe.edit.section.image": "Afbeelding",
    "recipe.edit.image_hint": "Plak een directe link naar een afbeeldingsbestand. Laat leeg om te verwijderen.",
    "recipe.edit.action.edit_image": "Afbeeldings-URL wijzigen",
    "recipe.edit.action.save_image": "Afbeeldings-URL opslaan",
    "recipe.edit.action.clear_image": "Wissen",
    "recipe.edit.image_none": "Geen afbeeldings-URL ingesteld.",
    "recipe.edit.aria.edit_image": "Afbeeldings-URL wijzigen",
    "recipe.edit.aria.save_image": "Afbeeldings-URL opslaan",
    "recipe.edit.aria.clear_image": "Afbeeldings-URL wissen",
    "recipe.edit.servings_hint": "Klik op Wijzigen om aan te passen hoeveel porties dit recept geeft.",
    "recipe.edit.action.edit_servings": "Wijzigen porties",
    "recipe.edit.action.save_servings": "Porties opslaan",
    "recipe.edit.aria.edit_servings": "Wijzigen porties",
    "recipe.edit.title_hint": "Klik op Wijzigen om dit recept een andere naam te geven.",
    "recipe.edit.action.edit_title": "Wijzig titel",
    "recipe.edit.action.edit_title_plain": "Titel wijzigen",
    "recipe.edit.ingredients_hint": "Gebruik de knoppen aan het eind van de regel om te wijzigen of verwijderen, of voeg onderaan een ingrediënt toe.",
    "recipe.edit.section.description": "Bereiding",
    "recipe.edit.description_hint": "Klik op Wijzigen om de bereidings te wijzigen.",
    "recipe.edit.action.edit_description": "Bereiding wijzigen",
    "recipe.edit.tags_hint": "Wijzig tags om te kunnen zoeken en filteren.",
    "recipe.edit.action.save_tags": "Tags opslaan",
    "recipe.edit.section.status": "Status",
    "recipe.edit.status_hint": "Wijzig status van dit recept.",
    "recipe.edit.section.delete": "Recept verwijderen",
    "recipe.edit.delete_hint": "Hiermee verwijder je dit recept en de bereiding.",
    "recipe.edit.action.delete": "Recept verwijderen",
    "recipe.edit.action.save_title": "Titel opslaan",
    "recipe.edit.action.save_description": "Bereiding opslaan",
    "recipe.edit.aria.view_recipe": "Recept bekijken",
    "recipe.edit.aria.edit_title": "Titel wijzigen",
    "recipe.edit.aria.edit_description": "Bereiding wijzigen",
    "recipe.edit.aria.save_tags": "Tags opslaan",
    "recipe.edit.aria.delete_recipe": "Recept verwijderen",
    "recipe.view.created_by": "Gemaakt door {username}",
    "recipe.view.by_owner": "Door {username}",
    "recipe.view.title_with_id": "#{recipe_id} {recipe_name}",
    "recipe.view.action.edit": "Recept wijzigen",
    "recipe.view.already_imported": "Staat al in je kookboek",
    "recipe.view.action.import": "Importeer in mijn kookboek",
    "recipe.view.action.add_to_week_menu": "Toevoegen aan weekmenu",
    "recipe.view.action.add_to_grocery_list": "Toevoegen aan boodschappenlijst",
    "recipe.random_public.lead": "Ontdek een willekeurig publiek recept.",
    "recipe.random_public.empty": "Er zijn nog geen publieke recepten.",
    "recipe.status.public_recipe": "Publiek recept",
    "recipe.status.enabled_recipe": "Recept meenemen in weekmenu",
    "recipe.delete.confirm_prompt": "Als je zeker weet dat je dit recept wilt verwijderen, klik hier.",
    "recipe.delete.confirm_before": "Als je zeker weet dat je dit recept wilt verwijderen",
    "recipe.delete.click_here": "klik hier.",
    # ingredient
    "ingredient.placeholder.quantity": "Hoeveelheid",
    "ingredient.placeholder.unit": "Eenheid",
    "ingredient.placeholder.name": "Naam",
    "ingredient.action.add": "Toevoegen",
    "ingredient.action.remove_line": "Verwijderen",
    "ingredient.aria.add": "Ingrediënt toevoegen",
    "ingredient.aria.edit": "Ingrediënt wijzigen",
    "ingredient.aria.remove": "Ingrediënt verwijderen",
    "ingredient.aria.cancel_edit": "Wijzigen ongedaan maken",
    # recipes_missing_tags
    "recipes_missing_tags.lead": "Zoek recepten die tags missen in één of meer groepen.",
    "recipes_missing_tags.hint": "Klik op een missende tag-groep om tags voor dat recept te kiezen en sla daarna op of annuleer.",
    "recipes_missing_tags.missing_groups": "Missende tag-groepen:",
    "recipes_missing_tags.empty": "Elk recept heeft minstens één tag in elke groep.",
    "recipes_missing_tags.action.save": "Tags opslaan",
    "recipes_missing_tags.action.cancel": "Annuleren",
    "recipes_missing_tags.aria.edit_group": "Tags toevoegen voor {group_name}",
    "recipes_missing_tags.aria.save": "Tags opslaan",
    "recipes_missing_tags.aria.cancel": "Tags bewerken annuleren",
    # recipes_missing_description
    "recipes_missing_description.lead": "Zoek recepten die nog een beschrijving missen en open ze om die in te vullen.",
    "recipes_missing_description.empty": "Elk recept heeft een beschrijving.",
    # week_menu
    "week_menu.lead": "Plan maaltijden voor de week. Pin recepten op dagen die je bevallen, of zoek zelf naar recepten.",
    "week_menu.start_week_on": "Start week op",
    "week_menu.action.randomize": "Week randomiseren",
    "week_menu.action.copy": "Weekmenu kopiëren",
    "week_menu.action.copy.aria": "Weekmenu naar klembord kopiëren",
    "week_menu.constraints.title": "Weekmenu-randomiseringsopties",
    "week_menu.constraints.hint": "Optionele regels voor randomiseren. Vastgezette dagen tellen mee voor beperkingen.",
    "week_menu.constraints.manage_button": "Beheer beperkingen",
    "week_menu.constraints.save": "Opties opslaan",
    "week_menu.constraints.aria.save": "Randomiseringsopties opslaan",
    "week_menu.constraints.none_active": "Geen actieve beperkingen.",
    "week_menu.constraint.ignore": "Negeren",
    "week_menu.constraint.same_tag": "Overal dezelfde tag",
    "week_menu.constraint.vary": "Afwisselen over de week",
    "week_menu.constraint.minimum": "Minstens N met tag",
    "week_menu.constraint.choose_tag": "Kies tag",
    "week_menu.constraint.aria.tag_for_group": "Tag voor {group_name}",
    "week_menu.constraint.aria.minimum_for_group": "Minimum aantal voor {group_name}",
    "week_menu.constraint.aria.minimum_days": "Minimum aantal dagen met tag voor {group_name}",
    "week_menu.day.no_recipe": "Geen recept gekozen",
    "week_menu.day.aria.move_up": "Verplaats {day_label} omhoog",
    "week_menu.day.aria.move_down": "Verplaats {day_label} omlaag",
    "week_menu.day.aria.move_up_short": "Omhoog",
    "week_menu.day.aria.move_down_short": "Omlaag",
    "week_menu.day.aria.servings_for": "Porties voor {day_label}",
    "week_menu.day.pin": "Dag vastzetten",
    "week_menu.day.unpin": "Dag losmaken",
    "week_menu.day.aria.clear": "Recept van dag wissen",
    "week_menu.day.aria.search": "Recept zoeken",
    "week_menu.day.search_label": "Recept zoeken",
    # grocery
    "grocery.lead": "Sorteer ingrediënten links per winkel en bekijk je lijsten rechts.",
    "grocery.back_to_week_menu": "Terug naar weekmenu",
    "grocery.section.add_groceries": "Boodschappen toevoegen",
    "grocery.action.add_weekly_groceries": "Wekelijkse boodschappen toevoegen",
    "grocery.origin.from_recipes": "Uit recept(en):",
    "grocery.origin.from_weekly": "Uit je wekelijkse boodschappen",
    "grocery.origin.manual": "Handmatig toegevoegd",
    "grocery.origin.from_inventory": "In je voorraad (gereserveerd)",
    "grocery.empty": "Nog geen ingrediënten. Genereer een lijst vanuit je weekmenu, voeg hierboven je eigen boodschappen toe, of voeg je wekelijkse boodschappen toe.",
    "grocery.section.to_sort": "Te sorteren",
    "grocery.to_sort.empty": "Alles is gesorteerd, gecontroleerd of al in huis.",
    "grocery.section.to_check": "Te controleren",
    "grocery.section.already_have": "Heb ik al",
    "grocery.to_check.empty": "Niets gemarkeerd om later te controleren.",
    "grocery.already_have.empty": "Nog niets gemarkeerd als al in huis.",
    "grocery.section.by_shop": "Per winkel",
    "grocery.by_shop.empty": "Wijs ingrediënten toe aan een winkel om hier je lijsten op te bouwen.",
    "grocery.action.mark_all": "Alles afvinken",
    "grocery.action.mark_all.confirm": "Alles afvinken?",
    "grocery.copy.title": "Kopiëren voor berichten",
    "grocery.clear_list": "Lijst wissen",
    "grocery.clear_list.confirm": "Lijst wissen?",
    "grocery.label.unassigned": "Niet toegewezen",
    "grocery.label.to_check": "Te controleren",
    "grocery.aria.actions_for_item": "Acties voor {item_name}",
    "grocery.aria.remove_from_to_check": "Verwijder uit te controleren",
    "grocery.aria.move_to_to_check": "Verplaats naar te controleren",
    "grocery.aria.remove_from_already_have": "Verwijder uit heb ik al",
    "grocery.aria.already_have": "Heb ik al",
    "grocery.aria.edit_amount": "Hoeveelheid bewerken",
    "grocery.aria.edit_amount_for": "Hoeveelheid bewerken voor {item_name}",
    "grocery.aria.quantity_for": "Hoeveelheid voor {item_name}",
    "grocery.aria.unit_for": "Eenheid voor {item_name}",
    "grocery.aria.save_amount": "Hoeveelheid opslaan",
    "grocery.generate.action": "Boodschappenlijst genereren",
    "grocery.generate.aria": "Boodschappenlijst genereren",
    "grocery.generate.confirm": "Boodschappenlijst bijwerken?",
    "grocery.generate.replace": "Vervangen",
    "grocery.generate.replace.title": "Lijst vervangen door weekmenu-boodschappen",
    "grocery.generate.replace.aria": "Lijst vervangen",
    "grocery.generate.add": "Toevoegen",
    "grocery.generate.add.title": "Weekmenu-boodschappen toevoegen aan huidige lijst",
    "grocery.generate.add.aria": "Toevoegen aan lijst",
    # tags
    "tags.lead": "Maak taggroepen en waarden aan voor het categoriseren van recepten.",
    "tags.section.add_group": "Taggroep toevoegen",
    "tags.placeholder.group_name": "bijv. seizoen",
    "tags.aria.add_group": "Groep toevoegen",
    "tags.section.existing_groups": "Bestaande groepen",
    "tags.field.group_name": "Groepsnaam",
    "tags.aria.rename_group": "Groep hernoemen",
    "tags.aria.save_group": "Groep opslaan",
    "tags.aria.delete_group": "Groep verwijderen",
    "tags.field.tag": "Tag",
    "tags.aria.rename_tag": "Tag hernoemen",
    "tags.aria.save_tag": "Tag opslaan",
    "tags.aria.delete_tag": "Tag verwijderen",
    "tags.field.new_value": "Nieuwe waarde",
    "tags.placeholder.tag_name": "bijv. zomer",
    "tags.aria.add_tag": "Tag toevoegen",
    # shops
    "shops.lead": "Maak winkels aan, stel hun kleuren in en wijs ingrediënten opnieuw toe.",
    "shops.section.add_shop": "Winkel toevoegen",
    "shops.placeholder.shop_name": "bijv. Albert Heijn",
    "shops.aria.add_shop": "Winkel toevoegen",
    "shops.section.existing_shops": "Bestaande winkels",
    "shops.aria.save_shop": "Winkel opslaan",
    "shops.aria.delete_shop": "Winkel verwijderen",
    "shops.empty": "Nog geen winkels.",
    "shops.assignments.title": "Ingrediënttoewijzingen",
    "shops.assignments.hint": "Wijs ingrediënten hier opnieuw toe. De boodschappenlijst wijst alleen een winkel toe als er nog geen is ingesteld.",
    "shops.assignments.unassigned": "Niet toegewezen",
    "shops.assignments.empty": "Nog geen ingrediënten. Voeg eerst recepten met ingrediënten toe.",
    "shops.assignments.in_recipes": "in {count} recept(en)",
    "shops.assignments.aria.delete_unused": "Ongebruikt ingrediënt verwijderen",
    "shops.assignments.aria.shop_for": "Winkel voor {ingredient_name}",
    "shops.assignments.aria.unassigned": "Niet toegewezen",
    # weekly_groceries
    "weekly_groceries.lead": "Houd een lijst bij van boodschappen die je elke week koopt en voeg ze in één klik toe aan een boodschappenlijst.",
    "weekly_groceries.back_to_grocery_list": "Terug naar boodschappenlijst",
    "weekly_groceries.section.add": "Wekelijkse boodschap toevoegen",
    "weekly_groceries.aria.add": "Wekelijkse boodschap toevoegen",
    "weekly_groceries.section.yours": "Jouw wekelijkse boodschappen",
    "weekly_groceries.aria.save": "Opslaan",
    "weekly_groceries.aria.delete": "Verwijderen",
    "weekly_groceries.empty": "Nog geen wekelijkse boodschappen. Voeg hierboven je vaste boodschappen toe.",
    "inventory.lead": "Houd bij wat je in huis hebt. Als je een boodschappenlijst maakt, gaan artikelen die volledig op voorraad zijn direct naar 'Heb ik al' en worden hier afgetrokken. Zet je ze terug op een lijst, dan komt de hoeveelheid terug.",
    "inventory.section.add": "Toevoegen aan voorraad",
    "inventory.aria.add": "Toevoegen aan voorraad",
    "inventory.section.yours": "Jouw voorraad",
    "inventory.sort.label": "Sorteer op",
    "inventory.sort.updated_desc": "Laatst bijgewerkt (nieuwste eerst)",
    "inventory.sort.updated_asc": "Laatst bijgewerkt (oudste eerst)",
    "inventory.sort.name": "Naam (A–Z)",
    "inventory.aria.save": "Opslaan",
    "inventory.aria.delete": "Verwijderen",
    "inventory.empty": "Je voorraad is leeg. Voeg hierboven toe wat je in huis hebt, of markeer artikelen op de boodschappenlijst als 'Heb ik al'.",
    # units
    "units.lead": "Beheer meeteenheden voor recepten en boodschappenlijsten. Elke eenheid heeft een afkorting, enkelvoud en meervoud.",
    "units.section.add": "Eenheid toevoegen",
    "units.placeholder.abbrev": "Afk.",
    "units.placeholder.singular": "Enkelvoud",
    "units.placeholder.plural": "Meervoud",
    "units.aria.abbreviation": "Afkorting",
    "units.aria.singular_label": "Enkelvoud",
    "units.aria.plural_label": "Meervoud",
    "units.aria.add_unit": "Eenheid toevoegen",
    "units.section.yours": "Jouw eenheden",
    "units.aria.abbreviation_for": "Afkorting voor {abbrev}",
    "units.aria.singular_for": "Enkelvoud voor {abbrev}",
    "units.aria.plural_for": "Meervoud voor {abbrev}",
    "units.section.recipes_using_unit": "Recepten die deze eenheid gebruiken",
    "units.empty": "Nog geen eenheden.",
    "units.empty.no_recipes_using_unit": "Nog geen recepten gebruiken deze eenheid.",
    # ingredient_units
    "ingredient_units.lead": "Vind ingrediënten die in meer dan één eenheid voorkomen in recepten of wekelijkse boodschappen. Zet ze om naar één eenheid met een conversiefactor.",
    "ingredient_units.section.pairs": "Ingrediënten met meerdere eenheden",
    "ingredient_units.empty": "Geen ingrediënten gebruiken meer dan één eenheid.",
    "ingredient_units.and": "en",
    "ingredient_units.action.convert": "Omzetten",
    "ingredient_units.aria.convert": "Eenheden omzetten",
    "ingredient_units.aria.amount_for": "Hoeveelheid voor {unit}",
    "ingredient_units.form.keep_unit": "Deze eenheid overal behouden",
    "ingredient_units.form.conversion": "Conversie",
    "ingredient_units.form.submit": "Conversie toepassen",
    "ingredient_units.form.cancel": "Annuleren",
    # Ingredient merge
    "ingredient_merge.lead": "Voeg twee ingrediënten samen die hetzelfde betekenen. Het doel-ingrediënt behoudt zijn naam; het andere wordt verwijderd.",
    "ingredient_merge.section.merge": "Ingrediënten samenvoegen",
    "ingredient_merge.form.hint": "Zoek het ingrediënt dat je wilt verwijderen en het ingrediënt waarvan je de naam wilt behouden. Klik op een suggestie om te selecteren. Receptregels met dezelfde eenheid worden gecombineerd.",
    "ingredient_merge.form.source": "Samenvoegen van (verwijderen)",
    "ingredient_merge.form.target": "Samenvoegen naar (naam behouden)",
    "ingredient_merge.form.search_placeholder": "Zoek ingrediënten…",
    "ingredient_merge.form.submit": "Ingrediënten samenvoegen",
    "ingredient_merge.recipe_count": "{count} recept(en)",
    "ingredient_merge.empty": "Nog geen ingrediënten. Voeg eerst recepten met ingrediënten toe.",
    # message
    "message.auth.invalid_credentials": "Ongeldige gebruikersnaam of wachtwoord.",
    "message.auth.username_required": "Gebruikersnaam is verplicht.",
    "message.auth.username_taken": "Die gebruikersnaam is al in gebruik.",
    "message.auth.password_min_length": "Wachtwoord moet minstens {min} tekens zijn.",
    "message.auth.passwords_no_match": "Wachtwoorden komen niet overeen.",
    "message.auth.email_updated": "E-mailadres bijgewerkt.",
    "message.auth.settings_updated": "Instellingen bijgewerkt.",
    "message.auth.current_password_incorrect": "Huidig wachtwoord is onjuist.",
    "message.auth.new_password_min_length": "Nieuw wachtwoord moet minstens {min} tekens zijn.",
    "message.auth.new_passwords_no_match": "Nieuwe wachtwoorden komen niet overeen.",
    "message.auth.password_changed": "Wachtwoord gewijzigd.",
    "message.recipe.already_imported": "Je hebt dit recept al geïmporteerd.",
    "message.recipe.all_days_pinned": "Alle dagen zijn vastgezet. Maak minstens één dag los voordat je een recept toevoegt.",
    "message.recipe.added_to_week_menu": "Toegevoegd aan weekmenu: {day}",
    "message.recipe.name_updated": "Receptnaam bijgewerkt",
    "message.recipe.name_not_saved": "Geen receptnaam gevonden, niet opgeslagen.",
    "message.recipe.image_url_updated": "Receptafbeelding bijgewerkt.",
    "message.recipe.image_url_cleared": "Receptafbeelding gewist.",
    "message.recipe.image_url_invalid": "Die afbeeldings-URL is niet geldig.",
    "message.recipe.image_url_scheme": "Afbeeldings-URL moet beginnen met http:// of https://.",
    "message.recipe.image_url_extension": "Afbeeldings-URL moet eindigen op .jpg, .jpeg, .png, .gif, .webp of .avif.",
    "message.recipe.image_url_credentials": "Afbeeldings-URL mag geen gebruikersnaam of wachtwoord bevatten.",
    "message.recipe.image_url_too_long": "Afbeeldings-URL is te lang.",
    "message.recipe.description_updated": "Receptbeschrijving bijgewerkt",
    "message.recipe.description_not_saved": "Geen receptbeschrijving gevonden, niet opgeslagen.",
    "message.recipe.invalid_quantity": "Ongeldige hoeveelheid, niet opgeslagen.",
    "message.recipe.no_quantity": "Geen hoeveelheid gevonden, niet opgeslagen.",
    "message.recipe.unit_not_found": "Eenheid niet gevonden, niet opgeslagen.",
    "message.recipe.no_unit": "Geen eenheid gevonden, niet opgeslagen.",
    "message.recipe.ingredient_duplicate": "Ingrediënt staat al in het recept. Niet opgeslagen.",
    "message.recipe.no_ingredient_name": "Geen ingrediëntnaam gevonden, niet opgeslagen.",
    "message.recipe.ingredient_updated": "Ingrediënt bijgewerkt",
    "message.recipe.no_ingredient_selected": "Geen ingrediënt geselecteerd.",
    "message.recipe.no_quantity_provided": "Geen hoeveelheid opgegeven.",
    "message.recipe.no_unit_selected": "Geen eenheid geselecteerd.",
    "message.recipe.unit_not_found_named": "Eenheid niet gevonden: {unit}",
    "message.recipe.ingredient_duplicate_edit": "Ingrediënt staat al in het recept. Bewerk het bestaande ingrediënt.",
    "message.recipe.ingredient_added": "Ingrediënt toegevoegd",
    "message.recipe.error": "Fout: {error}",
    "message.recipe.tags_updated": "Recepttags bijgewerkt",
    "message.recipes_missing_tags.tags_saved": "Tags voor {group_name} opgeslagen voor recept ",
    "message.recipe.deleted": "Recept verwijderd: {name}",
    "message.recipe.new_ingredient": "Hé, {name} kende ik nog niet!",
    "message.recipe.ingredient_not_added": "Ingrediënt niet toegevoegd: {name} (eenheid niet gevonden: {unit})",
    "message.recipe.already_in_grocery_list": "al op je boodschappenlijst",
    "message.recipe.ingredients_added": "Ingrediënten toegevoegd aan boodschappenlijst",
    "message.recipe.servings_updated": "Porties bijgewerkt",
    "message.recipe.servings_must_be_positive": "Porties moeten minstens 1 zijn",
    "message.recipe.servings_invalid": "Ongeldige portieswaarde",
    "message.recipe.servings_not_saved": "Porties niet opgeslagen",
    "message.week_menu.grocery_preserved": "Je boodschappenlijst is bewaard en niet opnieuw gegenereerd vanuit het weekmenu.",
    "message.week_menu.copied": "Weekmenu gekopieerd naar het klembord.",
    "message.week_menu.all_days_pinned_randomize": "Alle dagen zijn vastgezet. Maak minstens één dag los om te randomiseren.",
    "message.week_menu.enter_ingredient": "Voer een ingrediëntnaam in.",
    "message.week_menu.enter_positive_amount": "Voer een positieve hoeveelheid in.",
    "message.week_menu.added_grocery": "{name} toegevoegd aan je boodschappenlijst.",
    "message.week_menu.no_weekly_groceries": "Je hebt nog geen wekelijkse boodschappen. Voeg ze eerst toe via Instellingen.",
    "message.week_menu.weekly_already_on_list": "Je wekelijkse boodschappen staan al op de boodschappenlijst.",
    "message.week_menu.added_weekly_groceries": "{count} wekelijkse {noun} toegevoegd aan je boodschappenlijst.",
    "message.week_menu.noun_grocery": "boodschap",
    "message.week_menu.noun_groceries": "boodschappen",
    "message.week_menu.reserved_from_inventory": "{count} {noun} naar 'Heb ik al' verplaatst omdat je genoeg op voorraad hebt.",
    "message.week_menu.constraint_select_tag": "Selecteer een tag voor elke actieve tagbeperking voordat je randomiseert.",
    "message.week_menu.constraint_unsatisfied": "Kon geen weekmenu maken dat aan de geselecteerde tagbeperkingen voldoet.",
    "message.week_menu.combined_existing": "Samengevoegd met bestaande {name} ({unit}).",
    "message.tags.group_added": "Taggroep toegevoegd: {name}",
    "message.tags.group_exists": "Taggroep bestaat al: {name}",
    "message.tags.group_name_required": "Naam van taggroep is verplicht.",
    "message.tags.group_renamed": "Taggroep bijgewerkt naar: {name}",
    "message.tags.tag_added": "Tag '{tag}' toegevoegd aan {group}.",
    "message.tags.tag_exists": "Tag '{tag}' bestaat al in {group}.",
    "message.tags.tag_name_required": "Tagnaam is verplicht.",
    "message.tags.tag_renamed": "Tag bijgewerkt naar: {name}",
    "message.tags.tag_deleted": "Tag verwijderd: {name}",
    "message.tags.group_delete_has_tags": "Kan taggroep '{name}' niet verwijderen zolang er nog tags in zitten.",
    "message.tags.group_deleted": "Taggroep verwijderd: {name}",
    "message.shops.name_required": "Winkelnaam is verplicht.",
    "message.shops.already_exists": "Die winkel bestaat al.",
    "message.shops.added": "Winkel toegevoegd.",
    "message.shops.name_in_use": "Die winkelnaam is al in gebruik.",
    "message.shops.updated": "Winkel bijgewerkt.",
    "message.shops.deleted": "Winkel verwijderd.",
    "message.shops.assignment_saved": "Toewijzing opgeslagen.",
    "message.shops.ingredient_deleted": "Ingrediënt verwijderd.",
    "message.units.abbrev_required": "Afkorting is verplicht.",
    "message.units.not_found": "Eenheid niet gevonden.",
    "message.units.added": "Eenheid toegevoegd.",
    "message.units.updated": "Eenheid bijgewerkt.",
    "message.units.in_use": "Kan '{abbrev}' niet verwijderen zolang het in recepten of lijsten wordt gebruikt.",
    "message.units.deleted": "Eenheid verwijderd.",
    "message.ingredient_units.not_found": "Ingrediënt of eenhedenpaar niet gevonden.",
    "message.ingredient_units.invalid_target_unit": "Selecteer een van de eenheden om te behouden.",
    "message.ingredient_units.invalid_amounts": "Voer aan beide kanten een positieve hoeveelheid in.",
    "message.ingredient_units.nothing_to_convert": "Niets gebruikt de geselecteerde eenheid meer.",
    "message.ingredient_units.recipes_edited": "{count} recept(en) aangepast om eenheid {new_unit} te gebruiken in plaats van {old_unit}:",
    "message.ingredient_units.lists_updated": "{count} boodschappenregel(s) bijgewerkt.",
    "message.ingredient_merge.not_found": "Ingrediënt niet gevonden.",
    "message.ingredient_merge.same_ingredient": "Kies twee verschillende ingrediënten.",
    "message.ingredient_merge.invalid_selection": "Selecteer beide ingrediënten voordat je samenvoegt.",
    "message.ingredient_merge.completed": "{source_name} samengevoegd in {target_name}.",
    "message.ingredient_merge.recipes_edited": "{source_name} samengevoegd in {target_name} in {count} recept(en):",
    "message.weekly_groceries.ingredient_required": "Ingrediëntnaam is verplicht.",
    "message.weekly_groceries.positive_amount": "Voer een positieve hoeveelheid in.",
    "message.weekly_groceries.unit_required": "Een eenheid is verplicht.",
    "message.weekly_groceries.unit_not_found": "Eenheid niet gevonden: {unit}",
    "message.weekly_groceries.already_exists": "Die wekelijkse boodschap bestaat al.",
    "message.weekly_groceries.added": "Wekelijkse boodschap toegevoegd.",
    "message.weekly_groceries.not_found": "Wekelijkse boodschap niet gevonden.",
    "message.weekly_groceries.updated": "Wekelijkse boodschap bijgewerkt.",
    "message.weekly_groceries.deleted": "Wekelijkse boodschap verwijderd.",
    "message.inventory.ingredient_required": "Ingrediëntnaam is verplicht.",
    "message.inventory.non_negative_amount": "Voer een hoeveelheid van nul of meer in.",
    "message.inventory.unit_required": "Een eenheid is verplicht.",
    "message.inventory.unit_not_found": "Eenheid niet gevonden: {unit}",
    "message.inventory.added": "Toegevoegd aan voorraad.",
    "message.inventory.merged": "Samengevoegd met het bestaande voorraadartikel in die eenheid.",
    "message.inventory.not_found": "Voorraadartikel niet gevonden.",
    "message.inventory.updated": "Voorraadartikel bijgewerkt.",
    "message.inventory.deleted": "Voorraadartikel verwijderd.",
    "admin.users.lead": "Maak accounts aan voor mensen die je uitnodigt. Zij moeten het tijdelijke wachtwoord bij de eerste login wijzigen.",
    "admin.users.create_title": "Gebruiker aanmaken",
    "admin.users.create_action": "Gebruiker aanmaken",
    "admin.users.list_title": "Bestaande gebruikers",
    "admin.users.empty": "Nog geen gebruikers.",
    "admin.users.created": "Account aangemaakt voor {username}.",
    "admin.users.temp_password_title": "Tijdelijk wachtwoord",
    "admin.users.temp_password_lead": "Deel dit wachtwoord één keer met {username}. Het wordt niet opnieuw getoond.",
    "admin.users.temp_password_label": "Wachtwoord:",
    "admin.users.temp_password_warning": "Kopieer het nu. Nadat je deze pagina verlaat is het niet meer terug te vinden.",
    "admin.users.column.admin": "Beheerder",
    "admin.users.column.must_change": "Wachtwoord wijzigen verplicht",
    "admin.translations.lead": "Bewerk Engelse tekst en vertalingen voor elke UI-string.",
    "admin.translations.language": "Taal",
    "admin.translations.search_placeholder": "Zoek op sleutel of tekst…",
    "admin.translations.filter_groups": "Groepen",
    "admin.translations.incomplete_only": "Alleen onvolledig",
    "admin.translations.column.key": "Sleutel",
    "admin.translations.column.english": "Engels",
    "admin.translations.column.translation": "Vertaling",
    "admin.translations.action.save": "Opslaan",
    "admin.translations.saved": "Opgeslagen",
    "admin.translations.empty": "Geen vertalingen komen overeen met deze filters.",
    "admin.info.lead": "Runtime- en databasegegevens voor deze installatie.",
    "admin.info.section.database": "Database",
    "admin.info.section.runtime": "Runtime",
    "admin.info.label.database_backend": "Backend",
    "admin.info.label.database_url": "Verbinding",
    "admin.info.label.postgres_version": "PostgreSQL-versie",
    "admin.info.label.user_count": "Gebruikers",
    "admin.info.label.recipe_count": "Recepten",
    "admin.info.label.python_version": "Python",
    "admin.info.label.platform": "Platform",
    "admin.info.label.debug": "Debugmodus",
    "admin.info.debug.on": "Aan",
    "admin.info.debug.off": "Uit",
}
