...
@pytest.mark.asyncio
async def test_recipe_status_migration_is_idempotent(tmp_path: Path) -> None:
    """The recipe status migration should only add columns that are still missing."""
    db_file = tmp_path / "idempotent.sqlite3"
    sqlite3.connect(db_file).close()

    for migration_path in MIGRATION_FILES[:-1]:
        await _apply_migration(db_file, migration_path)

    connection = sqlite3.connect(db_file)
    connection.execute(
        'ALTER TABLE "recipe" ADD COLUMN "private" INT NOT NULL DEFAULT 1'
    )
    connection.execute(
        'ALTER TABLE "recipe" ADD COLUMN "enabled" INT NOT NULL DEFAULT 1'
    )
    connection.commit()
    connection.close()

    status_migration = _load_migration(MIGRATION_FILES[-1])
    client = SqliteClient(connection_name="default", file_path=str(db_file))
    sql = await status_migration.upgrade(client)
    await client.close()
    ...
    await _apply_migration(db_file, MIGRATION_FILES[-1])
