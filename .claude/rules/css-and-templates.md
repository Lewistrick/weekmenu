...
- **Name/amount/unit rows:** `.ingredient-input.item-row` with the buttons in
...
`@import` line in the hub. Place new rules in the correct feature file:
...
Two modes:
- **HTMX request:** pass `confirm_action`, `hx_target`, `hx_swap`, and
  optionally `hx_method` (default `post`; e.g. `delete`). The ✓ is a plain
  `type="button"` with the `hx-*` attributes (no `<form>`), so the partial is
  safe to use inside another form.
- **Plain link:** pass `confirm_href` (renders an `<a>`).

Optional: `trigger_icon` (only shown when truthy), `trigger_icon_only=true`
(icon-only trigger; `trigger_label` becomes its title/aria-label),
`trigger_class` (defaults to `grocery-clear-list-trigger`).
